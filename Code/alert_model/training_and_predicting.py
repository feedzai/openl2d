...
# DATA LOADING -------------------------------------------------------------------------------------
data = pd.read_csv(Path(__file__).parent/'../../Dataset/alert_data/Base.csv')
...
if not (os.path.exists(Path(__file__).parent/'../../Dataset/alert_model/best_model.pickle')):
...
os.makedirs('../../Dataset/alert_data/processed_data/', exist_ok = True)

deployment['model_score'] = model.predict_proba(deployment.drop(columns = 'fraud_bool'))[:,1]
deployment.to_parquet(Path(__file__).parent/'../../Dataset/alert_data/processed_data/BAF_alert_model_score.parquet')
...
file_to_store = open(Path(__file__).parent/"../../Dataset/alert_model/model_properties.pickle", "wb")
