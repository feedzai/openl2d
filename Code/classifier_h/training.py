...
for param_space_dic in os.listdir('./param_spaces/'):
    ...
    for initial in np.arange(init_train, init_train + 2, 0.2):
        ...
        os.makedirs(f'../../l2d_benchmarking/classifier_h/models/', exist_ok=True)
        
        if not (os.path.exists(f'../../l2d_benchmarking/classifier_h/models/model_{n}')):
...
for model in os.listdir('../../l2d_benchmarking/classifier_h/models/'):
    study = int(model.split('_')[-1])
    with open('../../l2d_benchmarking/classifier_h/models/' + model + '/history.yaml', 'r') as infile:
        param_hist = yaml.safe_load(infile)

    with open('../../l2d_benchmarking/classifier_h/models/' + model + '/config.yaml', 'r') as infile:
...
selected_model_path = f'../../l2d_benchmarking/classifier_h/models/model_{selec_ix}'
...
avg_cost_full_rej = (data_cfg['lambda']*fp + fn)/(tn+fp+fn+tp)
...
os.makedirs(f'../../l2d_benchmarking/classifier_h/selected_model/', exist_ok=True)

with open(f'../../l2d_benchmarking/classifier_h/selected_model/best_model.pickle', 'wb') as outfile:
    pickle.dump(model, outfile)

with open(f'../../l2d_benchmarking/classifier_h/selected_model/model_properties.yaml', 'w') as outfile:
